...
class Users extends Component {
  ...
  componentDidMount () {
    ...
    const socket = SockJS(Constants.backendUrlBase + 'stompwebsocket')
  ...
  scrollChatWindowDown () {
    document.getElementById('chatPanel').scrollTop =
      document.getElementById('chatPanel').scrollHeight
  }

  websocketMessageArrived (incoming) {
    const chatMessage = JSON.parse(incoming.body)
    console.log('Received: ', chatMessage)
    if (
      !this.state.showChatWindow ||
      this.state.currentuserToChatWith.id !== chatMessage.author.id
    ) {
        ...
        <table className='table'>
          <tbody>
            ...
            <tr>
              <td>
                <h3>
                  <span class='label label-default'>Motto</span>
                </h3>
              </td>
              <td>
                <h3>{user.motto}</h3>
            ...
            <tr>
              <td>
                <h3>
                  <span class='label label-default'>Web-page</span>
                </h3>
              </td>
              <td>
                <h3>
                  <a href={user.webPageUrl} target='_blank' rel='noreferrer'>
                    Go to {user.name}s web page
                  </a>
                </h3>
              </td>
            </tr>
  ...
  loadChatWindow (user) {
    ...
    fetch(
      Constants.backendRESTUrlBase + 'messages/chat?otherUser=' + user.name,
      {
        method: 'GET',
        credentials: 'include',
        headers: {
  ...
  chatTextEntered (event) {
    if (event.keyCode === 13) {
      ...
      this.sendChatToBackend(
        textEntered,
        this.state.currentuserToChatWith.name
      )
    }
  }
  ...
  sendChatToBackend (text, addressee) {
    fetch(Constants.backendRESTUrlBase + 'messages/chat', {
      ...
      body: JSON.stringify({ text, toUser: addressee })
    })
  }
  ...
  showUsers () {
    return this.userList.map((user) => (
      <tr key={user.id}>
        ...
        <td key={user.id + 3}>
          <button id={user.id + 'chatWith'} onClick={() => this.chatWith(user)}>
            Chat with {user.name}
          </button>
        </td>
        <td key={user.id + 4}>
          <button
            id={user.id + 'showContact'}
            onClick={() => this.showContactDetails(user)}
          >
        ...
        <td key={user.id + 5}>
          <button
            id={user.id + 'showProfile'}
            onClick={() => this.showProfile(user)}
          >
  ...
  renderChatWindow () {
    return this.messages.map((message, index) => (
      <p key={index + '_p'} className='chatPanelParagraph'>
  ...
  render () {
    return (
      <div className='App'>
        ...
        <div className={this.state.showChatWindow ? 'col-sm-8' : 'col-sm-12'}>
        ...
        <div
          className={this.state.showChatWindow ? 'col-sm-4' : 'invisible'}
          id='chatWindow'
        >
